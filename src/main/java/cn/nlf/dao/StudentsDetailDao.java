...
public interface StudentsDetailDao extends BaseDao<StudentsDetail> {
    ...
    @Select("<script>" +
            "insert into students_detail (sid,student_name,cid,class_name,date,week,start_time,end_time,duration,status,reason,create_time) values " +
            ...
            "#{item.startTime},#{item.endTime},#{item.duration},#{item.status},#{item.reason},#{item.createTime}" +
            ")" +
            "</foreach>" +
            "</script>")
    ...
    @Select("select count(*) from students_detail where cid = #{cid} and status = #{status}")
    Integer getCount(@Param("cid")Integer cid,@Param("status")Integer status);


    @Select("SELECT sd.student_name studentName,sd.class_name className,sd.delete_date deleteDate,sd.reason,c.fee,c.total from students_detail sd " +
